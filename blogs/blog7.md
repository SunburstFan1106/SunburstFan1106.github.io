...
1. 求最长的不下降子序列的长度 $s$（子序列中元素按照原序列的顺序且允许相等）。
2. 在每个元素只能使用一次的条件下，求最多可以选出多少个长度为 $s$ 的不下降子序列（同一元素最多出现在一个子序列中）。
3. 允许序列中的首元素 $x_1$ 和末元素 $x_n$ 可重复使用（其他元素仍只能用一次）的情况下，求最多能取出多少个**不同的**长度为 $s$ 的不下降子序列，其中两个子序列的下标序列不同即认为不同。
...
建议大家可以再看一下 [这篇博客](https://www.cnblogs.com/birchtree/p/12912607.html)。
